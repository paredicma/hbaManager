...
def copy_hba(remoteIP,remoteConfDir): ## OK -> return 0 , Not OK -> return 256
	copy_state=os.system('scp -p '+PG_LOCAL_SERVER_DIR+'pg_hba.conf  postgres@'+remoteIP+':'+remoteConfDir+'pg_hba.conf')
	if ( copy_state==0 ):
		return True
	else:
		return False
	...
	if( targetDB !=''):
		...
		if( userName !=''):
			...
			if( validIP(clientIP)):
				...
				logWrite(LOG_FILE,"!!! NOT valid Auth. Type entered :"+connType)					

		else:
			logWrite(LOG_FILE,"User Name parameter  be null !!!")	
	else:
